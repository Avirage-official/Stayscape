...
export async function GET(
  ...
  if (error) {
    console.error('[hotel-admin/requests/[taskId]/messages] Failed to fetch messages:', error);
...
export async function POST(
  ...
  if (error) {
    console.error('[hotel-admin/requests/[taskId]/messages] Failed to insert message:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }

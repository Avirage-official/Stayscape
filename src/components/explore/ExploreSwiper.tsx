...
// ─── Derive unique categories from a flat list of drill items ───────────────
...
export default function ExploreSwiper({
  ...
  const fetchDrillData = useCallback(async (
    ...
    try {
      ...
      if (sectionId === 'happening_now') {
        ...
        // Nearby (in_your_world) and future sections use places
  ...
  const drillToItem = useCallback((item: DrillPlaceCard | DrillEventCard, sectionId: string) => {
    ...
    setSelectedItem({ item: item as ExploreItem, contentType });
  }, []);

  const navigateBack = useCallback(() => {
    if (view.level === 0) return;
    if (view.level === 1) {
      // Back to L0 — restore section hero
  ...
  // Sync hero image when section changes at L0
  useEffect(() => {
    if (view.level === 0) {
      setHeroImageUrl(sections[activeIndex]?.image_url ?? null);
    }
  ...
  function handlePointerDown(e: React.PointerEvent) {
    if (view.level > 0) return; // disable swipe when drilled in
  ...
  // Hero image: either the drilled region image or the section image
  ...
  function renderMobileDrillContent() {
    ...
    if (active?.id === 'made_for_you') {
      return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px', padding: '24px 16px' }}>
          ...
          <button
            style={{ marginTop: '8px', background: 'none', border: '1px solid rgba(193,127,58,0.4)', borderRadius: '8px', padding: '8px 18px', color: '#C17F3A', fontFamily: "'DM Sans', sans-serif", fontSize: '12px', cursor: 'pointer', letterSpacing: '0.05em' }}
          >
          ...
          {drillCategories.map((cat) => (
            <button
              ...
              style={{
                ...
                padding: '12px 14px', marginBottom: '5px',
                background: 'rgba(250,248,245,0.06)', border: '1px solid rgba(250,248,245,0.08)',
                borderRadius: '12px', cursor: 'pointer', textAlign: 'left', boxSizing: 'border-box',
    ...
    // L0 default — standard items list
    return (
      <div style={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'none' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{
            ...
            animation: `hsSkeletonPulse 1.4s ${i * 0.15}s ease-in-out infinite`,
          }} />
        ))}
      </div>
    );
  ...
  return (
    ...
    >
      {/* Full-bleed hero background — cross-dissolve via layered images */}
      {displayHeroUrl && (
        <img
          key={displayHeroUrl}
          src={displayHeroUrl}
          ...
          style={{
            ...
            objectFit: 'cover',
            zIndex: 0,
            animation: 'hsDissolve 680ms cubic-bezier(0.25,0,0,1) both',
          }}
        />
      )}
      {nextHeroImageUrl && (
        <img
          key={`next-${nextHeroImageUrl}`}
      ...
      <div
        className="flex flex-col md:flex-row"
        style={{ position: 'relative', zIndex: 3, flex: 1, minHeight: 0, gap: '12px', display: 'flex' }}
      >
        {/* Hero card */}
        <div
              ...
              {sections.map((s, i) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => goTo(i)}
                    style={{
                      flex: 1, background: isActive ? '#C17F3A' : 'rgba(250,248,245,0.1)',
                      border: `1px solid ${isActive ? '#C17F3A' : 'rgba(250,248,245,0.15)'}`,
                      borderRadius: '12px', padding: '9px 4px', cursor: 'pointer',
                      transition: 'all 200ms ease', display: 'flex', flexDirection: 'column',
          ...
          >
            ...
            {view.level === 0 && (
              <button
                ...
                style={{
                  flexShrink: 0, marginTop: '10px', width: '100%', height: '40px',
                  borderRadius: '11px', background: '#C17F3A', color: '#FAF8F5',

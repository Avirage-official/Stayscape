 ...
 *   Housekeeping, Room Service, Restaurants & Bars, Laundry, Transportation,
 *   Wake-up Call, Call Staff, Maintenance, Luggage Pickup, Late Checkout
 *
...
const ALL_TILES: (Tile & { toggleKey?: keyof ServiceFlags })[] = [
  {
    ...
    label: 'Housekeeping',
    icon: <IconBed />,
    action: { kind: 'request', task_type: 'housekeeping', title: 'Housekeeping requested' },
    toggleKey: 'housekeeping_enabled',
  },
  {
    id: 'room_service',
    label: 'Room Service',
    ...
    toggleKey: 'room_service_enabled',
  },
  {
    id: 'restaurants_bars',
  ...
  },
  {
    ...
    toggleKey: 'transport_enabled',
  },
  {
    id: 'luggage_pickup',
    label: 'Luggage Pickup',
    icon: <IconLuggage />,
    action: { kind: 'request', task_type: 'luggage_pickup', title: 'Luggage pickup requested' },
    toggleKey: 'luggage_pickup_enabled',
  },
  {
    id: 'late_checkout',
    label: 'Late Checkout',
    ...
    toggleKey: 'late_checkout_enabled',
  },
  {
    id: 'wakeup',
    label: 'Wake-up Call',
...
export default function StayHomePage() {
  ...
  const buildDescription = (tile: Tile): string | undefined => {
    switch (tile.id) {
      ...
      case 'laundry': {
        const parts = [
          fields.laundryTime ? `Preferred pickup time: ${fields.laundryTime}` : null,
          fields.laundryNotes ? `Notes: ${fields.laundryNotes}` : null,
        ].filter(Boolean);
      ...
      case 'maintenance':
        return [
          fields.maintenanceIssue ? `Issue: ${fields.maintenanceIssue}` : null,
          fields.maintenanceNotes ? `Details: ${fields.maintenanceNotes}` : null,
        ].filter(Boolean).join(' | ') || undefined;
      case 'late_checkout':
        return fields.lateCheckoutTime ? `Desired checkout time: ${fields.lateCheckoutTime}` : undefined;
      case 'luggage_pickup':
        return fields.luggageNotes ? `Notes: ${fields.luggageNotes}` : undefined;
      default:
        return undefined;
    }
  };
    ...
    try {
      ...
      const res = await fetch('/api/customer/requests', {
        ...
        body: JSON.stringify({
          ...
          description: buildDescription(tile) ?? null,
        }),
      });
      if (!res.ok) {
  ...
  return (
    ...
    >
      ...
      <p
        ...
        style={{ margin: 0, fontSize: 'clamp(28px, 4vw, 38px)', fontWeight: 400, lineHeight: 1.15 }}
      >
        {greeting},{' '}
            ...
            <div className="tile-grid">
              {visibleTiles.map((tile) => (
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openTile(tile); }
                  }}
                >
        ...
        <div>
          ...
          <div className="ss-card-raised" style={{ padding: 22 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                <p className={cormorant.className} style={{ margin: 0, fontSize: 28, fontWeight: 400, lineHeight: 1, color: 'var(--text-primary)' }}>
                  Coming soon
                </p>
        ...
        <div style={{ marginTop: 36 }}>
          ...
          <div className="amenity-row">
            {places.slice(0, 4).map((p) => {
              ...
              return (
                <div
                  key={p.id} className="amenity-card ss-card-raised"
                  role="button" tabIndex={0}
                  onClick={() => router.push(`/stay/${propertySlug}/${stayId}/discover`)}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); router.push(`/stay/${propertySlug}/${stayId}/discover`); } }}
                >
                  ...
                  <div style={{ padding: '12px 14px 14px' }}>
                    ...
                    {p.category && (
                      <p style={{ margin: '2px 0 0', fontSize: 11, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>{p.category}</p>
                    )}
                  </div>
                </div>
              );
...
function svgProps() {
  return { width: 22, height: 22, viewBox: '0 0 24 24', fill: 'none' as const, stroke: 'currentColor', strokeWidth: 1.5, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const };
}
function IconBed() {
  return <svg {...svgProps()}><path d="M3 18v-7a3 3 0 013-3h12a3 3 0 013 3v7" /><path d="M3 14h18" /><path d="M3 18h18" /></svg>;
}
function IconFork() {
  return <svg {...svgProps()}><path d="M7 2v8a3 3 0 003 3v9" /><path d="M11 2v6" /><path d="M3 2v6a4 4 0 004 4" /><path d="M17 2v20M17 2c2 0 3 2 3 5v6h-3" /></svg>;
}
function IconDumbbell() {
  return <svg {...svgProps()}><path d="M6 8v8M3 10v4M18 8v8M21 10v4M6 12h12" /></svg>;
}
function IconLaundry() {
  return <svg {...svgProps()}><rect x="4" y="3" width="16" height="18" rx="2" /><circle cx="12" cy="13" r="4" /><path d="M8 6h.01M11 6h.01" /></svg>;
}
function IconCar() {
  return <svg {...svgProps()}><path d="M5 17h14M6 17l1-5h10l1 5M7 12l1.5-4h7L17 12" /><circle cx="7.5" cy="17.5" r="1.5" /><circle cx="16.5" cy="17.5" r="1.5" /></svg>;
}
function IconLuggage() {
  return <svg {...svgProps()}><rect x="6" y="7" width="12" height="14" rx="2" /><path d="M9 7V5a2 2 0 014 0v2" /><path d="M12 11v4" /><path d="M10 13h4" /></svg>;
}
function IconClock() {
...
function IconKey() {
  return <svg {...svgProps()}><circle cx="7.5" cy="15.5" r="5.5" /><path d="M21 2l-9.6 9.6" /><path d="M15.5 7.5l3 3L21 8l-3-3" /></svg>;
}
function IconInfo() {
  return <svg {...svgProps()}><circle cx="12" cy="12" r="9" /><path d="M12 8v.01M12 11v5" /></svg>;
}
function IconCloud() {
